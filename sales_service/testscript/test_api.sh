...
check_response() {
    ...
    case "$expected_type" in
        "token")
            ...
            ;;
        "customer")
            [[ "$response" == *"\"id\""* ]] && success "Customer operation successful" && return 0 || error "Customer operation failed" && return 1
            ;;
        "sale")
...
RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "$SALES_SERVICE_URL/customers/" \
    -H "Authorization: Bearer $AUTH_TOKEN" \
    -H "Content-Type: application/json" \
    -d '{"first_name": "Test", "last_name": "Customer", "email": "customer@example.com", "phone": "1234567890", "address": "123 Test Street"}')

print_response "POST /customers/" "$RESPONSE"
BODY=$(echo "$RESPONSE" | sed '$d')
CUSTOMER_ID=$(extract_id "$BODY")
test_endpoint_echo "Customer creation" "Customer ID: $CUSTOMER_ID"

RESPONSE=$(curl -s -w "\n%{http_code}" -H "Authorization: Bearer $AUTH_TOKEN" "$SALES_SERVICE_URL/customers/")
print_response "GET /customers/" "$RESPONSE"

RESPONSE=$(curl -s -w "\n%{http_code}" -H "Authorization: Bearer $AUTH_TOKEN" "$SALES_SERVICE_URL/customers/$CUSTOMER_ID")
print_response "GET /customers/$CUSTOMER_ID" "$RESPONSE"

RESPONSE=$(curl -s -w "\n%{http_code}" -X PUT "$SALES_SERVICE_URL/customers/$CUSTOMER_ID" \
    -H "Authorization: Bearer $AUTH_TOKEN" \
    -H "Content-Type: application/json" \
    -d '{"first_name": "Updated", "last_name": "Customer", "email": "updated@example.com", "phone": "1234567890", "address": "456 Updated Ave"}')
print_response "PUT /customers/$CUSTOMER_ID" "$RESPONSE"

RESPONSE=$(curl -s -w "\n%{http_code}" -X DELETE "$SALES_SERVICE_URL/customers/$CUSTOMER_ID" \
    -H "Authorization: Bearer $AUTH_TOKEN")
print_response "DELETE /customers/$CUSTOMER_ID" "$RESPONSE"
